...
class Var(object):
    ...
    BOT_TOKEN = str(getenv('BOT_TOKEN' , '7098978512:AAHMV8j-r5YBSCryvWHIkMrE8vtNbbo7bxw'))
    name = str(getenv('name', 'N2cccheckerbot'))
    SLEEP_THRESHOLD = int(getenv('SLEEP_THRESHOLD', '60'))
    WORKERS = int(getenv('WORKERS', '4'))
    BIN_CHANNEL = int(getenv('BIN_CHANNEL', '-1002056617893'))
    NEW_USER_LOG = int(getenv('NEW_USER_LOG', '-1002056617893'))
    PORT = int(getenv('PORT', '8080'))
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', '0.0.0.0'))
    PING_INTERVAL = int(environ.get("PING_INTERVAL", "1200"))  # 20 minutes
    OWNER_ID = [int(x) for x in os.environ.get("OWNER_ID", "6302921275").split()]
    ...
    if 'DYNO' in environ:
        ...
        ON_HEROKU = False
    FQDN = str(getenv('FQDN', 'BIND_ADRESS:PORT')) if not ON_HEROKU or getenv('FQDN', 'https://dying-quintina-n2bots-5cd35652.koyeb.app/') else APP_NAME+'.herokuapp.com'
